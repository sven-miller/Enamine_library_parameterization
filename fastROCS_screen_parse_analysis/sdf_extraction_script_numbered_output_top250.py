...
def process_csv(csv_file, sdf_directory, output_directory, rows_to_process=250):
    # Read CSV file into a DataFrame
    df = pd.read_csv(csv_file)

    # Process only the first 'rows_to_process' rows
...
def main():
    ...
    parser.add_argument('--rows', type=int, default=251, help='Number of rows to process (default: 250)')

    # Parse command-line arguments
    args = parser.parse_args()

    # Process the CSV file and generate SDF files
    process_csv(args.csv_file_path, args.sdf_directory, args.output_directory, args.rows)
