...
input_directory = "/expanse/lustre/scratch/millers7/temp_project/Enamine_NP_library_tautomer_output/"

# Define the submit template
submit_template = """#!/bin/bash
#SBATCH --job-name="fpk"
#SBATCH --output="fixpka.%j.%N.out"
...
chunks = list(chunk_list(file_paths, 350))

# Create a job file for each chunk
for i, chunk in enumerate(chunks):
    job_file_name = f"fixpka_job_{i + 1}.sb"
    ...
    for file_path in chunk:
        ...
        output_name = os.path.splitext(os.path.basename(file_path))[0] + "__fixpka.smi"
        
        # Define the command template with input and output paths
        command_template = f"python /home/millers7/scratch/fixpka_job_printer/setreasonablechargestate.py {file_path} /expanse/lustre/scratch/millers7/temp_project/Enamine_NP_library_fixpka_output/{output_name}"
