...
def main(argv=[__name__]):
    ...
    for qfname in args.query:
        ...
        while oechem.OEReadMolecule(qfs, mcmol):
            ...
            new_dbase= dbname.split('/')[-1].split('__tautomers')[0]
            moltitle = mcmol.GetTitle().split('.')[0]
            if len(moltitle) == 0:
                moltitle = str(qmolidx)
            ofname = new_dbase + '__' + moltitle + "_results." + ext
            #Outputs files to individual directories, MUST BE MAKE ADHEAD OF RUN!
            if not ofs.open(args.prefix + '/' + moltitle + '/' + ofname):
