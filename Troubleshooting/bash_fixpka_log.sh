...
output_file="fixpka_results.txt"
...
do
  ...
  num_molecules=$(python molcount.py ../Enamine_NP_library_fixpka_output/"$file" | grep -Eo '[0-9]+' | awk '{s+=$1} END {print s}')
  
  # Append the file name and the number of molecules to the output file
  echo "$file,$num_molecules" >> "$output_file"
done < job_outputs_fixpka.txt
